...
class _Calculator2State extends State<Calculator2> {
  final TextEditingController _offerSubjectController = TextEditingController();
...
{"id": "naprawy_miejscowe", "label": "naprawa ewentualnych uszkodzeń powierzchni, miejscowe naprawy", "type": "fixed", "price": 0.0},
    ...
    "Nałożenie warstwy bazowej żywicy": [
      ...
      {"id": "bazowa_nalozenie", "label": "nałożenie bazowej warstwy żywicy przy użyciu narzędzi do aplikacji", "type": "fixed", "price": 0.0},
    ],
    "Nałożenie warstwy końcowej żywicy": [
      ...
      {"id": "koncowa_nalozenie", "label": "nałożenie końcowej warstwy żywicy przy użyciu narzędzi do aplikacji", "type": "fixed", "price": 0.0},
    ...
    "Prace dodatkowe": [
      ...
      {"id": "doklejanie_plytek", "label": "doklejanie pojedynczych płytek", "type": "fixed", "price": 0.0},
    ],
  ...
  final osobaOptions = ["Marek", "Iwona",];
...
void _calculate() {
  ...
  double maxRabat = (selectedOsoba == "Iwona" 
  // || selectedOsoba == "Roman"
  ) ? 15.0 : 100.0;
  ...
  return Padding(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(
          "Koszt materiałów: ${result.totalPrice.toStringAsFixed(2)} zł brutto",
          style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black),
        ),
        if (_lastRabat > 0)
          Text(
            "Rabat na materiały: $_lastRabat%",
            style: const TextStyle(fontSize: 16, color: Colors.black),
          ),
        if (_lastRabat > 0)
          Text(
            "Koszt materiałów po rabacie: ${sumaPoRabacie.toStringAsFixed(2)} zł brutto",
            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black),
          ),
          ...
          return Column(
            ...
            children: [
              if (showExecution) ...[
                ...
                Text(
                  "Koszt wykonania: ${execAfterDiscount.toStringAsFixed(2)} zł brutto",
                  style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black),
                ),
                ...
                const Divider(color: Colors.orange),
                Text(
                  "Do zapłaty łącznie: ${grandTotal.toStringAsFixed(2)} zł brutto",
...
final telPrzygotowujacego = osobaInfo["tel"] ?? '';
    ...
    final double grandTotal = sumaPoRabacie + (showExecution ? execAfterDiscount : 0.0);
        ...
        // Dzielenie długiej listy zakresu na pierwszą stronę i resztę
    final List<String> __labelsAll = _selectedExecutionLabels().toList();
    const int __FIRST_PAGE_BULLET_LIMIT = 10; // możesz zwiększyć, jeśli masz więcej miejsca na 1 stronie
    final List<String> __labelsFirst = __labelsAll.length > __FIRST_PAGE_BULLET_LIMIT
        ? __labelsAll.sublist(0, __FIRST_PAGE_BULLET_LIMIT)
        : __labelsAll;
    final List<String> __labelsRest = __labelsAll.length > __FIRST_PAGE_BULLET_LIMIT
        ? __labelsAll.sublist(__FIRST_PAGE_BULLET_LIMIT)
        : <String>[];

// --- Grupowanie zakresu wykonania wg kategorii (z limitem na 1. stronę) ---
final List<pw.Widget> __execFirst = <pw.Widget>[];
final List<pw.Widget> __execRest = <pw.Widget>[];
int __remain = __FIRST_PAGE_BULLET_LIMIT;

_executionOptions.forEach((cat, items) {
  final List<String> selected = [];
  for (final item in items) {
    final id = item["id"] as String;
    if (_selectedExecution.contains(id)) {
      selected.add(item["label"] as String);
    }
  }
  if (selected.isEmpty) return;

  void addHeaderTo(List<pw.Widget> target, {bool big = false}) {
    target.add(pw.SizedBox(height: 4));
    target.add(pw.Text(cat, style: pw.TextStyle(font: arialBold, fontSize: big ? 10 : 9)));
    target.add(pw.SizedBox(height: 2));
  }

  if (__remain > 0) {
    // Header on first page
    addHeaderTo(__execFirst, big: false);
    int i = 0;
    for (final s in selected) {
      if (__remain > 0) {
        __execFirst.add(pw.Row(
    ...
    if (i < selected.length) {
      // Still items left -> repeat header on the continuation page
      addHeaderTo(__execRest, big: true);
      for (final s in selected.sublist(i)) {
        __execRest.add(pw.Row(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
            pw.Text('• ', style: pw.TextStyle(font: arial, fontSize: 10)),
            pw.Expanded(child: pw.Text(s, style: pw.TextStyle(font: arial, fontSize: 10))),
          ],
        ));
      }
    }
  } else {
    // No space left on first page -> everything goes to continuation
    addHeaderTo(__execRest, big: true);
    for (final s in selected) {
      __execRest.add(pw.Row(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
    ...
    // === WYBRANE PRODUKTY DO SEKCJI W PDF ===
    String? _findSelectedName(String prefix) {
      final key = _result!.packList.keys.firstWhere(
        (k) => k.startsWith(prefix),
        orElse: () => '',
      );
      if (key.isEmpty) return null;
      return key.split(':').last.trim();
    }

    final String? __hydroName = _findSelectedName('Hydroizolacja:');
    final String? __gruntName = _findSelectedName('Grunt:');
    final String? __mataName = _findSelectedName('Mata:');

    final List<String> __inne = [];
    _result!.packList.keys.forEach((k) {
      if (k.startsWith('Dodatek:')) {
        __inne.add(k.split(':').last.trim());
      } else if (k.startsWith('Naicoplast:')) {
        __inne.add(k.split(':').last.trim());
      } else if (k.startsWith('Płatki:')) {
        __inne.add('Płatki dekoracyjne');
      }
    });

    final String _ofertaDotyczy = _offerSubjectController.text.trim();
...
pw.SizedBox(height: 2),
   pw.Text('Wygenerowano: $dataWygenerowania',
                  ...
                  pw.SizedBox(height: 16),
                   pw.Text('Klient:', style: pw.TextStyle(font: arialBold, fontSize: 10)),
 ...
 pw.Text('Zremontowani.pl LTD',
               ...
               pw.Text(emailPrzygotowujacego, style: pw.TextStyle(font: arial, fontSize: 9)),
                ...
                pw.Divider(color: PdfColors.orange),
                 pw.Text('Wycena wykonania z materiałem', style: pw.TextStyle(font: arialBold, fontSize: 12)),
                 pw.SizedBox(height: 6),
                  if (__hydroName != null)
if (_ofertaDotyczy.isNotEmpty) pw.Text('Oferta dotyczy ' + _ofertaDotyczy + ' o powierzchni ' + powierzchnia.toString() + ' m2 systemem '+ __hydroName + '.',
                      style: pw.TextStyle(font: arial, fontSize: 9)),
                        pw.SizedBox(height: 8),
                // ===== WYBRANE PRODUKTY (bez ilości) =====
                pw.Text('Wybrane produkty', style: pw.TextStyle(font: arialBold, fontSize: 10)),
                pw.SizedBox(height: 4),
                if (__gruntName != null)
                  pw.Text('Grunt: ' + __gruntName, style: pw.TextStyle(font: arial, fontSize: 9)),
                if (__hydroName != null)
                  pw.Text('Hydroizolacja: ' + __hydroName, style: pw.TextStyle(font: arial, fontSize: 9)),
                if (__mataName != null)
                  pw.Text('Mata zbrojeniowa: ' + __mataName, style: pw.TextStyle(font: arial, fontSize: 9)),
                if (__inne.isNotEmpty)
                  pw.Text(
                    'Inne: ' + __inne.join(' , ') + ' oraz inne potrzebne produkty wynikające ze specyfiki prac dodatkowych',
                    style: pw.TextStyle(font: arial, fontSize: 9),
                  ),
                  pw.SizedBox(height: 6),
...
pw.Divider(color: PdfColors.orange),
                  ...
                  children: [
                    pw.Column(
                      ...
                      children: [
                        if (execRabat > 0)
                          pw.Text('Rabat: $execRabat%',
                              style: pw.TextStyle(font: arial, fontSize: 9)),
                              pw.SizedBox(height: 3),
                        pw.Text('Cena netto: ' + (grandTotal / 1.23).toStringAsFixed(2) + ' zł', style: pw.TextStyle(font: arialBold, fontSize: 10)),
                             pw.SizedBox(height: 3),
                        pw.Text('VAT 23%',
                            style: pw.TextStyle(font: arial, fontSize: 9)),
                             pw.SizedBox(height: 3),
pw.Text('Razem do zapłaty: ${grandTotal.toStringAsFixed(2)} zł brutto',
                        style: pw.TextStyle(font: arialBold, fontSize: 11, color: PdfColors.black),
                      ),
                      ],
                    )
                  ],
...
// KLIENT i POWIERZCHNIA
              ...
              //  pw.Text(emailPrzygotowujacego, style: pw.TextStyle(font: arial, fontSize: 9)),
                pw.SizedBox(height: 12),
                    pw.Text('Więcej informacji o technologii na www.zremontowani.pl',
                        style: pw.TextStyle(font: arialBold, fontSize: 9, color: PdfColors.orange)),
                                       pw.SizedBox(height: 12),
...
pw.Container(
  ...
  // Druga i kolejne strony z kontynuacją zakresu wykonania
  if (__labelsRest.isNotEmpty) {
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (ctx) => pw.Row(
          ...
          children: [
            pw.Text('Zakres wykonania – ciąg dalszy', style: pw.TextStyle(font: arialBold, fontSize: 12)),
            pw.Text('Strona ${ctx.pageNumber} / ${ctx.pagesCount}', style: const pw.TextStyle(fontSize: 9)),
          ],
        ),
        build: (ctx) => [
          pw.SizedBox(height: 6),
        ...
        footer: (ctx) => pw.Align(
          ...
          child: pw.Text('Strona ${ctx.pageNumber} / ${ctx.pagesCount}', style: const pw.TextStyle(fontSize: 9)),
        ),
      ),
    );
  }
  ...
  @override
  void dispose() {
    _offerSubjectController.dispose();
    super.dispose();
  }
    ...
    body: Stack(
      children: [
        ...
        const SizedBox(height: 16,),
                          ...
                          _decoratedField(
  child: TextFormField(
    controller: _offerSubjectController,
    decoration: const InputDecoration(
      labelText: "Oferta dotyczy:",
      hintText: "np. uszczelnienia dachu, hydroizolacji tarasu...",
      labelStyle: TextStyle(color: Colors.black),
      border: InputBorder.none,
    ),
    style: const TextStyle(color: Colors.black),
...
const SizedBox(height: 12),
                ...
                child: TextFormField(
                  ...
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  decoration: const InputDecoration(
                    labelText: "Rabat na materiały (max 15%)",
                    labelStyle: TextStyle(color: Colors.black),
                    border: InputBorder.none,
                  ),
                  style: const TextStyle(color: Colors.black),
